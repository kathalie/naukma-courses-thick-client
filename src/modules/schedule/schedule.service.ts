...
export class ScheduleService {
  ...
  private parsePage(page: any, seasonNumber: number): IScheduleItem[] {
    ...
    $('#schedule-accordion > div ').each((idx, elem) => {
  ...
  private parseFaculty(
    $: cherrio.CheerioAPI,
    faculty: cherrio.Element,
    ...
    $(faculty)
      .find(`#${accordionIdFaculty} > div`)
      .each((i, e) => {
        const specialtyHeading = $(e)
          .find('div:nth-child(1) > h4 > a')
          .text()
          .split(' ')
        ...
        const educationLevel: EducationLevel =
          educationLevelAbbreviation === 'БП' ? EducationLevel.BACHELOR : EducationLevel.MASTER;
        const idFacultyYear: string =
          idFaculty + '-' + (educationLevel === EducationLevel.BACHELOR ? 1 : 2) + '-' + year;
  ...
  private retrieveSpecialtyName(specialityName: string, educationLevelAbbreviation: string): string {
    const idx = specialityName.indexOf(educationLevelAbbreviation);
