...
export class CourseService {
  constructor(protected readonly feedbackService: CourseFeedbackService) {
  }
  ...
  private async scrapeData(url: string): Promise<ICourse> {
    ...
    listItems.each((idx, el) => {
      const textItems = $(el).find('th').text().trim()
        .split('\t');
      textItems.forEach((text) => {
        if (text == 'Весна') seasonsList.push(CourseSeason.SPRING);
        else if (text == 'Осінь') seasonsList.push(CourseSeason.AUTUMN);
        else if (text == 'Літо') seasonsList.push(CourseSeason.SUMMER);
      });
    });
    ...
    const rateAverage = await this.feedbackService.findRateAverageByCode(code);
